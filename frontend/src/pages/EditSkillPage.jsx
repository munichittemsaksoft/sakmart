...
export default function EditSkillPage() {
  ...
  useEffect(() => {
    if (skill) {
      reset({
        ...
        parameters: (skill.parameters || []).map(v => ({ value: v })),
        tags: skill.tags?.join(', ') || '',
        price: skill.price ? skill.price / 100 : '',
        status: skill.status,
      })
    }
  }, [skill, reset])
  ...
  const mutation = useMutation({
    mutationFn: (data) => skillApi.update(slug, data),
    onSuccess: (updated) => {
      toast.success('Skill updated!')
      qc.invalidateQueries(['skill', slug])
      navigate(`/skills/${updated.slug}`)
    },
  ...
  if (!isOwner && !isAdmin) {
    return <div className="text-center py-32 text-dark-700/50">Not authorized to edit this skill.</div>
  }
  ...
  function onSubmit(values) {
    mutation.mutate({
      ...
      price: values.price ? Math.round(Number(values.price) * 100) : null,
    ...
    <div className="max-w-3xl mx-auto px-4 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-sm font-semibold text-amber-600 uppercase tracking-wider">Edit Skill</span>
        </div>
        <h1 className="font-display font-bold text-3xl text-dark-950">{skill.name}</h1>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="card p-6">
          ...
          <div className="space-y-2">
            ...
            {paramFields.length === 0 && (
              <button type="button" onClick={() => appendParam({ value: '' })}
                className="text-xs text-dark-700/40 hover:text-amber-600">
                + Add a parameter
              </button>
            )}
          </div>
        ...
        <div className="flex gap-3">
          ...
          <button type="button" onClick={() => navigate(`/skills/${slug}`)} className="btn-outline px-6">
            Cancel
          </button>
        </div>
      </form>
