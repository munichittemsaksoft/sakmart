...
export default function CheckoutPage() {
  ...
  const buyMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Payment successful! You now own this template.')
  ...
  function validate() {
    ...
    if (digits.length !== 16) e.card_number = 'Must be 16 digits'
  ...
  if (!template.price || template.price <= 0) {
    return (
      <div className="text-center py-32 text-dark-700/50">
        This template is free — no checkout needed.{' '}
        <Link to={`/templates/${slug}`} className="text-primary-500 hover:underline">Go back</Link>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <Link
        to={`/templates/${slug}`}
        className="inline-flex items-center gap-1.5 text-sm text-dark-700/60 hover:text-primary-500 mb-8"
      >
        ...
        <div className="lg:col-span-3">
          <div className="card p-6">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 mb-6 flex items-start gap-2 text-sm text-amber-800">
              ...
              <span>
                <strong>Demo mode:</strong> This is a mock payment gateway. No real charges occur.
                Use any card number.
              </span>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  className={`input ${errors.cardholder_name ? 'border-red-400' : ''}`}
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    className={`input pr-10 font-mono tracking-wider ${errors.card_number ? 'border-red-400' : ''}`}
        ...
        <div className="lg:col-span-2">
          <div className="card p-5 sticky top-6">
            ...
            <div className="flex items-start gap-3 pb-4 border-b border-surface-border">
              <div className="w-10 h-10 rounded-lg bg-primary-50 flex items-center justify-center text-primary-600 font-bold text-lg shrink-0">
                {template.title[0]}
              </div>
              <div className="min-w-0">
                <p className="font-semibold text-dark-950 text-sm leading-snug line-clamp-2">{template.title}</p>
                <p className="text-xs text-dark-700/50 mt-0.5">by @{template.author?.username}</p>
                <span className="text-xs badge badge-blue mt-1 inline-block">{template.category}</span>
              </div>
            </div>

            <div className="space-y-2 py-4 border-b border-surface-border text-sm">
              <div className="flex justify-between">
                <span className="text-dark-700/60">Template price</span>
            ...
            <div className="flex justify-between pt-4 font-display font-bold text-dark-950">
              <span>Total</span>
              <span className="text-primary-600 text-lg">{formatPrice(template.price)}</span>
            </div>

            <div className="mt-4 space-y-1.5 text-xs text-dark-700/50">
              <p>✓ Instant access after payment</p>
              <p>✓ Download ZIP bundle</p>
              <p>✓ {template.agent_count} AI agent{template.agent_count !== 1 ? 's' : ''} included</p>
            </div>
          </div>
        </div>
      </div>
