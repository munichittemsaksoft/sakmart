...
import { Users, Layers, GitFork, Star, Settings, Shield, Lock } from 'lucide-react'
...
const ROLE_BADGE = {
  user:        'badge-gray',
  admin:       'badge-blue',
  super_admin: 'bg-violet-100 text-violet-700 inline-flex items-center gap-1 rounded-full text-xs font-semibold px-2.5 py-0.5',
}
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-accent-500 font-semibold text-xs uppercase tracking-wider">Control Panel</p>
            <h1 className="font-display font-bold text-3xl text-dark-950">Admin</h1>
          </div>
        </div>
        <span className={clsx(
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
        ...
        <div>
          <h2 className="font-display font-semibold text-lg flex items-center gap-2 mb-4">
            ...
            {!isSuperAdmin && <Lock size={13} className="text-dark-700/30 ml-1" />}
          </h2>
          {!isSuperAdmin ? (
            <div className="card p-6 text-center text-sm text-dark-700/40">
            ...
            <div className="card overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-surface-border">
                  {users?.items?.map((u) => {
                    const isSelfRow = u.id === currentUser.id
                    const targetIsSuperAdmin = u.role === 'super_admin' && !isSelfRow
                    return (
                      <tr key={u.id} className="hover:bg-surface-soft">
                        ...
                        <td className="px-3 py-2.5">
                          {targetIsSuperAdmin ? (
                            <span className="badge badge-gray text-xs opacity-60">super_admin</span>
                          ) : (
                            <select
                              value={u.role}
                        ...
                        <td className="px-3 py-2.5 text-center">
                          <button
                            onClick={() => !targetIsSuperAdmin && activeMut.mutate({ id: u.id, active: !u.is_active })}
                            disabled={targetIsSuperAdmin}
                            className={clsx(
                              'badge cursor-pointer',
                              u.is_active ? 'badge-green' : 'badge-gray',
                              targetIsSuperAdmin && 'opacity-40 cursor-not-allowed',
                            )}
                          >
                            {u.is_active ? 'Yes' : 'No'}
                          </button>
        ...
        <div>
          <h2 className="font-display font-semibold text-lg flex items-center gap-2 mb-4">
            <Layers size={18} className="text-primary-500" /> All Templates
          </h2>
          {tplLoading ? <Spinner /> : (
            <div className="card overflow-hidden">
              <table className="w-full text-sm">
                    ...
                    <tr key={t.id} className="hover:bg-surface-soft">
                      <td className="px-4 py-2.5">
                        <Link to={`/templates/${t.slug}`}
                          className="font-medium text-dark-950 hover:text-primary-500 line-clamp-1">
                          {t.title}
                        </Link>
                      ...
                      <td className="px-3 py-2.5 text-right">
                        <button
                          onClick={() => { if (confirm('Delete this template?')) tplDeleteMut.mutate(t.id) }}
