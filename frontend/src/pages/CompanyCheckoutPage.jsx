...
import { ArrowLeft, CreditCard, Lock, ShieldCheck, Building2, Users } from 'lucide-react'
...
export default function CompanyCheckoutPage() {
  ...
  const buyMutation = useMutation({
    mutationFn: () => companyApi.buy(slug, {
    ...
    onSuccess: () => {
      toast.success('Payment successful! You now own this company setup.')
  ...
  if (!company.price || company.price <= 0) {
    return (
      <div className="text-center py-32 text-dark-700/50">
        This setup is free.{' '}
        <Link to={`/companies/${slug}`} className="text-primary-500 hover:underline">Go back</Link>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <Link to={`/companies/${slug}`} className="inline-flex items-center gap-1.5 text-sm text-dark-700/60 hover:text-primary-500 mb-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button type="submit" disabled={buyMutation.isPending}
                ...
                {buyMutation.isPending
                  ...
                  : <><Lock size={16} /> Pay {formatPrice(company.price)}</>}
              </button>
            </form>
          ...
          <div className="card p-5 sticky top-6">
            ...
            <div className="flex items-start gap-3 pb-4 border-b border-surface-border">
              ...
              <div className="min-w-0">
                <p className="font-semibold text-dark-950 text-sm leading-snug line-clamp-2">{company.name}</p>
                {company.industry && <p className="text-xs text-dark-700/50 mt-0.5">{company.industry}</p>}
                <p className="text-xs text-dark-700/40 mt-0.5">by @{company.author?.username}</p>
              </div>
            </div>

            <div className="space-y-2 py-4 border-b border-surface-border text-sm">
              <div className="flex justify-between">
                <span className="text-dark-700/60">Setup price</span>
                <span className="font-medium">{formatPrice(company.price)}</span>
              </div>
            ...
            <div className="flex justify-between pt-4 font-display font-bold text-dark-950">
              <span>Total</span>
              <span className="text-primary-600 text-lg">{formatPrice(company.price)}</span>
            </div>

            <div className="mt-4 space-y-1.5 text-xs text-dark-700/50">
              <p>✓ Complete company AI team</p>
              <p>✓ {company.agent_count} AI agent{company.agent_count !== 1 ? 's' : ''} included</p>
              {company.zip_url && <p>✓ Downloadable configuration</p>}
            </div>
          </div>
        </div>
      </div>
